...
async function createSetsAndFill(ctx) {
    ...
    for (const [format, list] of byFormat.entries()) {
        for (let chunkIndex = 0; chunkIndex * MAX_STICKERS_PER_SET < list.length; chunkIndex++) {
            ...
            const first = chunk[0];
            const attemptedIds = new Set(chunk.map((c) => c.fileId));
            let created = false;
            try {
...
async function createCustomEmojiSets(ctx) {
    ...
    if (ctx.session.emojiCollectMode === "full_sets") {
        ...
        for (const name of setNames) {
            try {
                ...
                if (ctx.session.debug)
                    await ctx.reply(`DEBUG: загрузил набор ${name}: +${set.stickers.length}`);
            }
            catch { }
        }
        expanded = full.length ? full : expanded;
        if (ctx.session.debug)
            await ctx.reply(`DEBUG: после expand по наборам → ${expanded.length}`);
    }
    ...
    if (ctx.session.debug)
        await ctx.reply(`DEBUG: items сформировано: ${items.length}`);
    ...
    const deduped = byFile;
    if (ctx.session.debug)
        await ctx.reply(`DEBUG: после dedup fileId → ${deduped.length}`);
    const byFormat = new Map();
    ...
    for (const [format, list] of byFormat.entries()) {
        if (ctx.session.debug)
            await ctx.reply(`DEBUG: формат ${format}, элементов: ${list.length}`);
        for (let chunkIndex = 0; chunkIndex * MAX_STICKERS_PER_SET < list.length; chunkIndex++) {
            const chunk = list.slice(chunkIndex * MAX_STICKERS_PER_SET, (chunkIndex + 1) * MAX_STICKERS_PER_SET);
            ...
            for (const it of chunk.slice(1)) {
                try {
                    const input = { emoji_list: [it.emoji || "❤️"], sticker: it.fileId, format: sticker_format };
                    ...
                    skipped += 1;
                    if (ctx.session.debug)
                        await ctx.reply(`DEBUG: skip index=${added + skipped} причина=${e?.description || e?.message || 'error'}`);
                }
            }
            ...
            try {
                ...
                if (verify.stickers.length < chunk.length) {
                    ...
                    if (ctx.session.debug)
                        await ctx.reply(`DEBUG: verify count=${verify.stickers.length}/${chunk.length}, retry add missing≈${missing}`);
                    ...
                    while (current < chunk.length && pass < 3) {
                        for (const it of chunk.slice(1)) {
                            ...
                            try {
                                const input = { emoji_list: [it.emoji || "❤️"], sticker: it.fileId, format: sticker_format };
                                await ctx.api.raw.addStickerToSet({ user_id: userId, name: short, sticker: input });
                                await sleep(150);
                    ...
                    if (ctx.session.debug)
                        await ctx.reply(`DEBUG: verify after retry count=${current}/${chunk.length}`);
                    added = Math.min(chunk.length, current);
                }
                // Final diff report
                try {
                    const latest = await ctx.api.getStickerSet(short);
                    const present = new Set(latest.stickers.map((s) => s.file_id));
                    const missingList = [];
